...
def find_free_port():
    ...
    s.bind(('', 0))  # Demande au système un port libre
    port = s.getsockname()[1]
    s.close()
    return port


def filtrer_donnees(df, date, value, choix):
    if date is not None :
        if len(date) != 0:
            df['temps'] = pd.to_datetime(df['temps'], errors='coerce',dayfirst=True)
            dates = [datetime.strptime(d, '%Y-%m-%d').date() for d in date]
            df_filtré = pd.DataFrame()
            for datej in dates:
                df1 = df[df['temps'].dt.date == datej]
                df_filtré = pd.concat([df_filtré, df1])
            if not df_filtré.empty:
                df = df_filtré

    if value != 'Aucun' and ((df is not None) and ('Heure' in df.columns) and (choix != 'COP_loi_deau_df')):
        # Convertir la colonne 'Heure' en datetime
        df['Heure'] = pd.to_datetime(df['Heure'], format='%H:%M:%S')

        # Créer une colonne pour l'intervalle de temps
        if value == '30min':
            df['Intervalle'] = df['temps'].dt.floor('30T')  # Arrondir à l'intervalle de 30 minutes
        elif value == '60min':
            df['Intervalle'] = df['temps'].dt.floor('H')  # Arrondir à l'intervalle de 1 heure
        
        # Garder uniquement la première valeur de chaque intervalle
        df = df.drop_duplicates(subset='Intervalle', keep='first')
        
        # Supprimer la colonne 'Intervalle' qui n'est plus nécessaire
        df = df.drop(columns=['Intervalle'])
    if value == 'Aucun':
        df = df
    return df
